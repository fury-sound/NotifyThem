...
final class MainSenderViewModel: ObservableObject {
    ...
    init() {
        ...
        //        print("MainSenderViewModel INIT", ObjectIdentifier(self))
        Task { await loadInitialData() }
    }

    func loadInitialData() async {
        //        print("in loadInitialData")
        //        let raw = try? await firestoreService.fetchReceiverGroupsRaw()
        //        print("groups in DB:", raw?.map { "\($0.id): \($0.name), receivers: \($0.receiverIds)" } ?? [])
        do {
            ...
            //            let (groups, receivers) = try await (groups, receivers)
            //            print("groups:", receiverGroupList)
    ...
    func addNewMessage(_ message: String, to groupID: UInt) {
        ...
        let newMessage = MessageCore(
            ...
            message: message,
            //            senderID: 1, // TODO: заменить на currentSender.id
        ...
        Task {
            ...
            //            print("in addNewMessage async", try? await firestoreService.fetchMessages(groupID: groupID).map { "\($0.id). \($0.message) \($0.date)" }.joined(separator: ", "))
        }
    }
    ...
    func updateReceiver(_ receiver: Receiver) {
        ...
        Task {
            do {
                ...
                //                print("in updateReceiver async", try? await firestoreService.fetchReceivers().map { "\($0.id). \($0.name)" }.joined(separator: ", "))
            } catch {
    ...
    func addNewReceiver(_ name: String) {
        ...
        Task {
            do {
                ...
                //                print("in addNewReceiver async", try? await firestoreService.fetchReceivers().map { "\($0.id). \($0.name)" }.joined(separator: ", "))
            } catch {
    ...
    func deleteReceiver(_ receiver: Receiver) {
        ...
        Task {
            do {
                ...
                //                print("in deleteReceiver async", try? await firestoreService.fetchReceivers().map { "\($0.id). \($0.name)" }.joined(separator: ", "))
            } catch {
    ...
    func deleteReceiverGroup(_ group: ReceiverGroup) {
        ...
        //        Task {
        //            print("1. BEFORE: in deleteReceiverGroup async", try? await firestoreService.fetchReceiverGroups().map { "\($0.id). \($0.name)" }.joined(separator: ", "))
        //        }
        ...
        Task {
            do {
                ...
                //                print("2. AFTER: in deleteReceiverGroup async", try? await firestoreService.fetchReceiverGroups().map { "\($0.id). \($0.name)" }.joined(separator: ", "))
            } catch {
